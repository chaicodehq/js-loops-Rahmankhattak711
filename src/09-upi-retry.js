...
export function upiRetry(outcomes) {
  ...
  do {
    ...
    const outcome = idx < outcomes.length ? outcomes[idx] : "fail";
    if (outcome === "success") {
      success = true;
      break;
    }

    // failed attempt: if we will retry (attempts < 5), add wait
    if (attempts < 5) {
      const wait = waitTimes[Math.min(attempts - 1, waitTimes.length - 1)];
      totalWaitTime += wait;
    }
  } while (attempts < 5);
